...
export default function ChargerDetail() {
  ...
  const postChatRoom = () => {
    ...
    myChatApi
      .postChatRoom(data)
      .then((res) => {
        navigate(`/chat-list/${res.chatRoomId}`);
      })
      .catch(() => {
  ...
  function handleLike() {
    ...
    if (charger && charger.favorite) {
      deleteFavorite(chargerId);
    }
    if (charger && !charger.favorite) {
      createFavorite(chargerId);
    }
  }
  ...
  function LikeButton() {
    return (
      <IconButton
        icon={charger?.favorite ? "like" : "emptyLike"}
        onClick={handleLike}
      />
    );
  }
  ...
  return (
    <S.ChargerContainer>
      <TopNavigationBar
        ...
        rightBtn={charger?.myChargerCheck ? <MoreIconButton /> : <LikeButton />}
      />
      <S.ChargerOverview>
        <S.ChargerStatus>
